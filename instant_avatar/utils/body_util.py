...
def _rvec_to_rmtx(rvec):
    ''' apply Rodriguez Formula on rotate vector (3,)
    ...
    return cos(theta)*np.eye(3) + sin(theta)*skew_mtx + (1-cos(theta))*np.dot(r, r.T)



def body_pose_to_body_RTs(jangles, tpose_joints):
    """ Convert body pose to global rotation matrix R and translation T.
    ...
    Rs[0] = _rvec_to_rmtx(jangles[0, :].cpu())
    
    Ts = np.zeros(shape=[total_joints, 3], dtype='float32')
    Ts[0] = tpose_joints[0, :].cpu()

    for i in range(1, total_joints):
        Rs[i] = _rvec_to_rmtx(jangles[i, :].cpu())
        Ts[i] = tpose_joints[i, :].cpu() - tpose_joints[SMPL_PARENT[i], :].cpu()
    
    return torch.from_numpy(Rs).cuda(), torch.from_numpy(Ts).cuda()
